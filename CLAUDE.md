...
- **Stage 4: 7th系コードタイプ対応（v1.14.0）**: 発見モードStage 5（v1.10.0）で確立したパターン（コードタイプ数を増やしても`_nextChord()`の動的リトライ・可解性チェックは無改修で対応できる設計）をそのまま横展開。`CHORD_TYPE_IDS`を`['maj', 'min']`から全10種類に拡大するだけで対応でき、`_nextChord()`のdo-whileリトライ・`hasSolvableChordTones()`呼び出しは想定通り無改修で済んだ（`type.chord`という汎用的な半音配列を渡すだけの構造だったため）
  ...
  - **対称コード（dim7・aug）に関する申し送り**: 同じ構成音セットが複数のルート候補から生成されうる点は発見モードと同じ制約だが、アルペジオモードは「聞いた音の順序の記憶と再現」が目的で判定は`pc`一致ベースで機械的に行われるため、機能的な支障はないと判断（構成音の同定を目的とする発見モードとは影響の性質が異なる）
  - **今回のスコープ外（今後の検討事項）**:
    - **テンション対応**: アルペジオモードは「フェーズ管理」方式で音の順番を記憶・再現する形式のため、テンションを足すと記憶負荷が上がるという発見モードとは異なる影響がある。まず7th系拡大単体の影響を見てから別途検討
