...
- **【解消済み】オクターブ無視ルールとテンション判定の整合性確認（2026-07-07）**: 長年の懸案事項だった「オクターブ無視ルールがテンション判定を無効化していないか」は解消済み。理由：発見モード（`js/chordGame.js`）にはテンション判定ロジック自体がそもそも存在しない。`music.js`の`CHORD_TYPES.tensions`フィールド（生の半音数、9th:14・b9:13等）はどこからも読まれていない未使用データで、実際の判定は`type.chord`（ピッチクラス、0〜11の範囲）と`isChordToneHit()`（純粋なpc一致）のみで行われている。オクターブを厳密に見る`isTensionHit()`は存在するが、`js/game.js`（インターバル編）専用で`chordGame.js`・`js/arpeggioGame.js`からは呼ばれていない。確定方針（コードトーン編＝オクターブ無視が正しい設計）と現状の実装は完全に整合しており、修正不要。**【将来テンション実装時の設計メモ】**: 実際にテンション対応を実装する際は、`isTensionHit()`のようなオクターブ厳密判定を流用するのではなく、既存の7th系/aug/sus系と同じ仕組みで`type.chord`にピッチクラス化した値（9th→2、b9→1など）を加える形にすること。`tensions`フィールドの生の半音数はそのままでは使えない（削除はせず将来の再設計時の参考としてそのまま残す）。インターバル編のオクターブ厳格化は引き続き別課題として切り離したまま
