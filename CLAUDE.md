...
インターバル編 v1.4.2 完了・GitHub Pages 公開済み（従来の改善に加え、ハンバーガーメニュー表示バグの修正、詰まった時のヒント表示機能（LV.Max除く、ON/OFF切替可）を追加。ヒントドットの半径は実機確認とフィードバックを経て 0.2→0.8→0.32 と調整し、正解フィードバックドットと同サイズに落ち着いた）。
