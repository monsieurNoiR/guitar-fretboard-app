...
js/
  ...
  arpeggioGame.js コードトーン編〔アルペジオモード〕ゲームロジック（ArpeggioGameクラス、Stage 2）
css/style.css
...
### コードトーン編〔アルペジオモード〕Stage 2（`js/arpeggioGame.js`）
...
- **出題**: ルート C 固定・低音3弦（6〜4弦）固定・トライアド（maj/min）のみ。7th系は対象外。`hasSolvableChordTones()` をそのまま流用（順序は可解性判定に無関係）
